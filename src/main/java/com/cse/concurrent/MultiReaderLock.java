...
public class MultiReaderLock {
	...
	private class ReadLock implements SimpleLock {
		...
		public void unlock() throws IllegalStateException {
			synchronized (lock) {
				...
				readers--;
				// notify all waiting threads if no active readers
				// lock requires notify to recheck condition and wake up waiting threads
				if (readers == 0) {
					lock.notifyAll();
				}
			}
	...
	private class WriteLock implements SimpleLock {
		...
		public void lock() {
			synchronized (lock) {
				try {
					// during the run, if the thread doesn't meet the condition, it will run
					// normally
					// but if the condition is met, it will wait
					// and lock will not stop waiting if there's no notify
					while ((writers > 0 || readers > 0) && !isActiveWriter()) {
						lock.wait();
					}
					writers++;
					// cite: ChatGPT to understand about currentThread
		...
		public void unlock() throws IllegalStateException, ConcurrentModificationException {
			synchronized (lock) {
				if (writers == 0) {
					throw new IllegalStateException("No active writers");
				}
				...
				writers--;
				if (writers == 0) {
					// unsetting the active writer to avoid 2 threads waiting for the lock
					activeWriter = null;
					lock.notifyAll();
				}
			}
