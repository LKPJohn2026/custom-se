...
public class HtmlCleaner {
	...
	public static String stripTags(String html) {
		// Cite: regex101.com for testing regular expressions
		String singleTag = "a|b|i|p|q|s|u";
		return html.replaceAll("(?is)</?(?:[a-z][a-z0-9]|[A-Z]{2,}[0-9]*|" + singleTag + ")(?:\\s+[^>]*?)?/?>|<!DOCTYPE[^>]*>|<\\?xml[^>]*\\?>", "");
	}
	...
	public static String stripElement(String html, String name) {
		// Cite: regex101.com for testing regular expressions
		String regex = String.format("(?is)<%1$s\\b[^>]*?>.*?(?:</%1$s\\b[^>]*?>|$)", name);
