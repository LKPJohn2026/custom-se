...
public class HtmlFetcher {
	...
	public static String fetch(URI uri, int redirects) {
		...
		try (
			...
			// 200 means success, then we fetch the html content
			if (statusCode == 200 && isHtml(headers)) {
				// read the response, gather into a string
				html = response.lines().collect(Collectors.joining(System.lineSeparator()));
			}
			// 300-399 means redirect, then we follow the redirect
			else if (statusCode >= 300 && statusCode < 400) {
				String location = getRedirect(headers);
				if (location != null && redirects > 0) {
					html = fetch(new URI(location), redirects - 1);
				}
			}
		}
